...
class ComparativaCotizacionesContrato extends Rotation
{
     ...
     * @var array
     */
    var $contratos, $cotizaciones, $subtotales;
    ...
    function Header()
    {
        ...
        for ($i = 0; $i < self::MAX_COTIZACIONES_PP && $i < ($this->num_cotizaciones - $this->acumuladas); $i++) {
            ...
            $this->Cell(($this->w - 2) * (0.7 / (2 * self::MAX_COTIZACIONES_PP)), 0.3, utf8_decode('P.U.'), 'BR', 0, 'C');
            $this->Cell(($this->w - 2) * (0.7 / (2 * self::MAX_COTIZACIONES_PP)), 0.3, utf8_decode('IMPORTE'), 'BR', 0, 'C');
    ...
    protected function encabezados() {
        ...
        $this->Cell(($this->w - 2) * 0.200, 0.35, utf8_decode('Descripción del subcontrato:'), '', 0, 'R');
        ...
        $this->Cell(($this->w - 2) * 0.8, 0.35, '#' . $this->contrato_proyectado['numero_folio'], 'TB', 2, 'L');
    ...
    protected function items() {
        foreach ($this->contratos as $index => $contrato) {
            ...
            $row = [
                $index + 1,
                str_repeat('----', strlen($contrato['nivel']) / 4) . utf8_decode($contrato['descripcion']),
                $contrato['unidad'] ? number_format($contrato['cantidad_presupuestada'], 2, '.', ',') : '',
                $contrato['unidad']
            ];

            for($i = 0; $i < self::MAX_COTIZACIONES_PP && $i < ($this->num_cotizaciones - $this->acumuladas); $i++) {

                $presupuesto = head(array_where($this->cotizaciones[$i + $this->acumuladas]['presupuestos'], function ($key, $value) use ($contrato) {
                    return $value['id_concepto'] == $contrato['id_concepto'];
                }));

                if(! is_null($presupuesto['IdMoneda']))
                    $this->subtotales[$i + $this->acumuladas][$presupuesto['IdMoneda']] += ($presupuesto['precio_unitario'] * $contrato['cantidad_presupuestada']);

                array_push($widths, ($this->w - 2) * (0.7 / (2 * self::MAX_COTIZACIONES_PP)));
                array_push($widths, ($this->w - 2) * (0.7 / (2 * self::MAX_COTIZACIONES_PP)));
