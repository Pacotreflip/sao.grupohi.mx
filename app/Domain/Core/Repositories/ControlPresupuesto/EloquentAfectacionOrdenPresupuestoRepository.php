<?php
/**
 * Created by PhpStorm.
 * User: EMARTINEZ
 * Date: 24/01/2018
 * Time: 11:57 AM
 */

namespace Ghi\Domain\Core\Repositories\ControlPresupuesto;


use Ghi\Domain\Core\Contracts\ControlPresupuesto\AfectacionOrdenPresupuestoRepository;
use Ghi\Domain\Core\Contracts\ControlPresupuesto\EstatusRepository;
use Ghi\Domain\Core\Models\ControlPresupuesto\AfectacionOrdenesPresupuesto;
use Ghi\Domain\Core\Models\ControlPresupuesto\Estatus;

class EloquentAfectacionOrdenPresupuestoRepository implements AfectacionOrdenPresupuestoRepository
{
    /**
     * @var \Ghi\Domain\Core\Models\ControlPresupuesto\Estatus
     */
    protected $model;

    /**
     * EloquentTipoOrdenRepository constructor.
     * @param \Ghi\Domain\Core\Models\ControlPresupuesto\Estatus $model
     */
    public function __construct(AfectacionOrdenesPresupuesto $model)
    {
        $this->model = $model;
    }

    /**
     * Obtiene todos los registros del estatus
     *
     * @return \Illuminate\Database\Eloquent\Collection|\Ghi\Domain\Core\Models\ControlPresupuesto\AfectacionOrdenesPresupuesto
     */
    public function all()
    {
        return $this->model->get();
    }

    /**
     * Buscar conceptos
     * @param $attribute
     * @param $operator
     * @param $value
     * @return \Illuminate\Database\Eloquent\Collection|AfectacionOrdenesPresupuesto
     */
    public function getBy($attribute, $operator, $value)
    {
        return $this->model->where($attribute, $operator, $value)->get();
    }

    /**
     * @param $relations
     * @return $this
     */
    public function with($relations) {
        $this->model = $this->model->with($relations);
        return $this;
    }


}