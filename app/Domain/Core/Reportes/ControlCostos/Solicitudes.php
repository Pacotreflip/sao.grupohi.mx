...
class Solicitudes extends Rotation {
    ...
    function firmas() {
        ...
        if (in_array($this->solicitud->estatus, [-1,2]))
        {
            $nombre = $this->solicitud->estatus == 2 ? $this->solicitud->autorizacion : $this->solicitud->rechazo;

            $this->Cell(($this->GetPageWidth() - 4) / 2, 0.4, utf8_decode('Solicitó'), 'TRLB', 0, 'C', 1);
            $this->Cell(2);
            $this->Cell(($this->GetPageWidth() - 4) / 2, 0.4, utf8_decode($this->solicitud->estatus == 2 ? 'Autorizó' : 'Rechazó'), 'TRLB', 1, 'C', 1);
            ...
            $this->Cell(2);
            $this->Cell(($this->GetPageWidth() - 4) / 2, 0.4, utf8_decode($nombre ? $nombre->usuario : ''), 'TRLB', 0, 'C', 1);
        }

        // Solicitada
        else
        {
            $this->Cell(($this->GetPageWidth() - 4) / 2, 0.4, utf8_decode('Solicitó'), 'TRLB', 1, 'C', 1);

            $this->Cell(($this->GetPageWidth() - 4) / 2, 1.2, '', 'TRLB', 1, 'C');

            $this->Cell(($this->GetPageWidth() - 4) / 2, 0.4, utf8_decode($this->solicitud->usuario), 'TRLB', 1, 'C', 1);
        }
